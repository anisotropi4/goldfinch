...
def get_schedule(schedule_list):
    ...
    for event in schedule_list:
        ...
        if 'Scheduled Arrival' in event:
            this_event = event.copy()
            this_event['Offset'] = this_event['Arrival Offset']
            this_event.pop('Scheduled Departure', None)
            this_event.pop('Public Departure', None)
            this_event.pop('Arrival Offset', None)
            this_event.pop('Departure Offset', None)
            this_event['Event'] = {'LI': 'A', 'LT': 'T'}[event_id]
            events.append(this_event)

        if 'Scheduled Departure' in event:
            this_event = event.copy()
            this_event['Offset'] = 'PT00:00:00'
            if 'Departure Offset' in this_event:
                this_event['Offset'] = this_event['Departure Offset']
            this_event.pop('Scheduled Arrival', None)
            this_event.pop('Public Arrival', None)
            this_event.pop('Arrival Offset', None)
            this_event.pop('Departure Offset', None)
            this_event['Event'] = {'LI': 'D', 'LO': 'O'}[event_id]
            events.append(this_event)

        if 'Scheduled Pass' in event:
            this_event = event.copy()
            this_event['Offset'] = this_event['Pass Offset']
            this_event.pop('Pass Offset', None)
            this_event['Event'] = 'P'
            events.append(this_event)
