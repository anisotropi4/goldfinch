...
def get_date_dt(date_object):
    return datetime(date_object.year, date_object.month, date_object.day, tzinfo=timezone.utc)
...
def solrdt_str(this_dt):
    return this_dt.strftime('%Y-%m-%dT%H:%M:%SZ')

#fin = open(fin_name, 'r', encoding='utf-8')
#WTT = pd.DataFrame([json.loads(i) for i in fin])
WTT = pd.DataFrame(get_data('PA', 'active/{}/{}'.format(solrdt_str(START_INTERVAL), solrdt_str(END_INTERVAL))))

WTT = WTT.fillna(value={'Origin': '00:00:00', 'Terminus': '00:00:00'})

df1 = WTT[['Start_Date', 'End_Date', 'Origin', 'Actual']].copy()
...
df1['d0_bitmap'] = df1['Actual'].apply(day_int)
